...
import {DNFValue, drawTableBoolFunction, getRandomVector, getValueINDNF, parseDNF} from "../utils/boolsUtils";
...
export default function Task6() {
    ...
    function onInputDNF(value: string) {
        ...
        const res = parseDNF(value);
        console.log("DNF", res.value.value);
        errors.current.DNF = res.error;
        commitErrors();

        if (res.error)
            return;
        if (nValue < res.value.maxPow) {
            errors.current.DNF = `Количество переменных в ДНФ больше чем в векторе - ${res.value.maxPow}`;
            commitErrors();
            return;
        }

        dnfRef.current = res.value.value;
    }
